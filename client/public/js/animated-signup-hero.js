(function () {
  ...
  var sharedStyles = `
    ...
    .signup-hero__message { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; opacity: 0; transform: translateY(10px); will-change: transform, opacity; animation: signup-hero-message 8.4s cubic-bezier(0.22, 1, 0.36, 1) infinite; animation-delay: calc(var(--slot) * 1.2s); }
    @keyframes signup-hero-message { 0% { opacity: 0; transform: translateY(10px); } 4%, 11% { opacity: 1; transform: translateY(0); } 14%, 100% { opacity: 0; transform: translateY(-10px); } }
    .signup-content { width: min(100%, 460px); margin: 0 auto; padding: 26px max(7vw, 28px) 0; }
    .form-eyebrow { display: none; }
    .form-title { margin: 0 0 5px; color: #000; font: 700 20px Arial, Helvetica, sans-serif; letter-spacing: -0.3px; line-height: 1.2; }
    ...
    .checkbox-row { color: #707070; font-size: 13px; }
    .checkbox-row input[type=checkbox] { accent-color: #c41e3a; }
    .checkbox-row a, .form-switch a { color: #b01e36; font-weight: 700; }
    .btn-submit { padding: 13px; border-radius: 8px; background: #c41e3a; box-shadow: 0 4px 14px rgba(196,30,58,.22); font: 700 13px Arial, Helvetica, sans-serif; }
    .btn-submit:hover { background: #a91731; box-shadow: 0 6px 18px rgba(196,30,58,.28); }
  ...
  function AnimatedSignupHero(element) {
    ...
    element.innerHTML = '<div class="signup-hero__messages" aria-hidden="true">' + messages.map(function (message, index) {
      return '<span class="signup-hero__message" style="--slot:' + index + '">' + message + '</span>';
    }).join('') + '</div>';
  }
